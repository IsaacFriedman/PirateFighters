...
public class FirstPersonController : MonoBehaviour
{
    ...
    // Camera tilt (slope) effect variables
    ...
    private void Update()
    {
        ...
        // Smoothly tilt the camera joint for slope effect

        if (joint != null)
        {
            Quaternion targetRotation = Quaternion.Euler(jointOriginalPos + targetTilt);
            joint.localRotation = Quaternion.Slerp(joint.localRotation, targetRotation, Time.deltaTime * tiltSpeed);
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        float tiltAmount = 3.5f;

        Vector3 localVelocity = transform.InverseTransformDirection(rb.linearVelocity);
        float tiltZ = -localVelocity.x / walkSpeed * tiltAmount; // Left/right strafe tilt
        float tiltX = localVelocity.z / walkSpeed * tiltAmount;  // Forward/back tilt
